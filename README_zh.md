...
- 写入白名单、预期输出、callback 行为，以及错误或受攻击 Worker 的影响范围。
...
默认的 `--callback auto` 只写 Inbox。正常完成、失败、取消、超时和请求输入
...
`--callback codex-resume` 仍作为显式兼容选项保留。它会启动独立、只读的
`codex exec resume` 进程。进程成功退出只记录为 `callback_executed`，不能记录
为 `delivered`，因为这无法证明当前可见的 IDE/App 对话收到了消息。只有返回
明确 acknowledgement 的宿主 adapter 才能把事件标为已确认/已送达。
