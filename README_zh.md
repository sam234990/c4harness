...
> 写入持久 C4 Inbox，并由 Dashboard 提醒用户。可解释的任务拆解与确定性的
> 公开的 `graph-run` CLI 现已支持执行编译后的任务图，包括依赖感知的受限
> 并行、结构化验证失败，以及每节点最多两次尝试。自动能力学习和更多 harness
> 支持仍在 Roadmap 中。
...
| 任务图 Runtime | **原型** | 公开 `graph-run`、依赖调度、受限并行 batch、节点验证、History outcome 与 Root Verification |
| 图作用域 Integration Workspace | **原型** | Git 无关快照、事务性 patch apply/verify/commit，每个任务图共享规范 integration workspace |
| 结构化 Verifier 阶段 | **原型** | Proposal 阶段与 post-integration 阶段检查，附带结构化失败分类 |
| 重试与 Fallback 执行 | **原型** | 最多两次总尝试；可重试失败使用同 worker 或下一 eligible fallback；环境/策略/冲突不重试 |
| Graph-run CLI | **可用** | 编译 version-1 proposal 并执行其已分配任务图；默认 dry-run |
| 受限并行执行 | **原型** | 独立 ready node 可并发，写集合重叠则串行，integration 与源仓库隔离 |
| 基于能力的任务图路由 | **原型** | 执行编译后的 worker assignment 与 eligible fallback；单任务 `run` 仍显式选择 |
...
`setup` 会在 `$XDG_DATA_HOME/c4harness/memory.sqlite3`（未设置时为
`~/.local/share/c4harness/memory.sqlite3`）创建个人全局账本，并将 Skill
安装到 `$HOME/.agents/skills/c4harness`。命令会输出需要加入
...
安装默认保留；使用 `c4harness setup --force` 更新安装副本。
...
`~/.config/c4harness/workers.json`（或 `C4HARNESS_WORKERS`），同一份配置可在
...
```bash
c4harness run --worker-id claude-mimo-pro --goal "审查这个模块" --path src/ --json
```
...
`graph-run` 使用与 `decompose` 相同的 version-1 proposal，然后执行已经分配的
节点。默认只做安全预览；只有显式传入 `--execute` 才会调用 Worker。
...
保守默认值是 `max-parallel=1`。只有依赖已经成功、且声明的写集合互不重叠时，
ready node 才会进入同一个并行 batch。Worker 调用可以重叠；patch integration、
post-integration verifier 和 commit 在任务图 workspace 中使用短事务串行完成。
系统不会自动修改源仓库。
...
网络接口，可运行 `c4harness dashboard --host 0.0.0.0`。Dashboard 没有
身份验证，不要将该端口暴露到不可信网络。
...
`--path` 为只读输入。`--write-path` 构成完整的写入白名单。Worker 仅在该次运行中获得 `Edit/Write` 工具。C4Harness 将工作区与基线对比，拒绝越界变更，输出 `proposed.patch`；不会自动应用 patch。
...
所有命令默认使用全局账本。可以通过 `C4HARNESS_MEMORY` 或
`--memory /path/to/memory.sqlite3` 指定其他账本，例如项目账本
`.c4harness/memory.sqlite3`。
...
- [x] 按依赖顺序执行任务契约图，并支持下游阻断、节点验证、History outcome 与 Root Verification。
- [x] 增加公开的 proposal 任务图执行入口，并对相互独立的 ready node 实现受限并行执行。
- [ ] 将能力、用户偏好、History、上下文规模、风险与策略评分接入 runtime 的自动 Worker 选择。
- [x] 将结构化 verifier failure 接入两次尝试预算、fallback 链与有界重规划决策。
- [ ] 定义 Inbox 保留与提醒策略。
...
- [x] 实现确定性的 contract-aware 节点验证与 Root Contract 验证。
- [x] 根据能力匹配、History evidence、verifier 可用性与候选差距输出可解释的 assignment confidence。
- [ ] 加入可插拔的测试、lint、类型检查和 patch 可应用性 verifier。
- [x] 实现 verifier 驱动的返工与升级循环。
