...
本文描述 C4Harness 的系统级组合关系。各模块的正式方法边界分别见：

- [decompose.md](decompose.md)：任务理解、契约图、worker assignment、VerifierPlan 和重规划。
- [delegator.md](delegator.md)：同步/异步 worker 执行与 harness adapter。
- [verifier.md](verifier.md)：节点、集成与 Root Contract 的实际验证。
- [memory.md](memory.md)：单次任务内的 Shared Task Memory。
- [history.md](history.md)：跨任务执行事实、Token 和能力证据。

系统整体流程由 Application 组合这些模块；任何单一方法文档都不应声称自己实现完整端到端生命周期。
...
> 完整方法见 [decompose.md](decompose.md)。该文档只定义 Task Situation、
> Task Contract Graph、能力约束 assignment、VerifierPlan 和反馈驱动的重规划。
> Worker 执行、实际验证、最终验收和历史记录不属于 Decompose。
...
6. Shared Memory 与 Execution History 明确分离：前者服务当前协作，后者保存可审计 outcome 并派生路由证据。
