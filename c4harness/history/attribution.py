...
from ..core.contracts import FailureCategory, VerificationResult, WorkerResult
...
def _classify_block_reasons(issues: list[str]) -> FailureAttribution:
    ...
    for issue in issues:
        lower = issue.lower()
        if "integration_conflict" in lower:
            return FailureAttribution.INTEGRATION_CONFLICT
...
def _classify_structured_failures(
    verification: VerificationResult,
) -> tuple[OutcomeStatus, FailureAttribution] | None:
    """Use verifier-native failure records before legacy issue parsing."""
    # ``file_exists`` is a deterministic observation, but a missing path can
    # be missing task context rather than evidence that the selected worker is
    # incapable.  Keep the verifier's deterministic category intact while
    # excluding this case from negative worker capability evidence.
    if any(
        failure.category == FailureCategory.DETERMINISTIC_REJECTION
        and failure.phase_or_check in {"file_exists", "file_contains"}
        and "does not exist" in failure.message.lower()
        for failure in verification.failures
    ):
        return OutcomeStatus.BLOCKED, FailureAttribution.MISSING_CONTEXT
    categories = {failure.category for failure in verification.failures}
    if not categories:
        return None
    if FailureCategory.POLICY_PERMISSION in categories:
        return OutcomeStatus.BLOCKED, FailureAttribution.PERMISSION_BLOCKED
    if FailureCategory.ENVIRONMENT in categories:
        return OutcomeStatus.BLOCKED, FailureAttribution.ENVIRONMENT_FAILURE
    if FailureCategory.INTEGRATION_CONFLICT in categories:
        return OutcomeStatus.BLOCKED, FailureAttribution.INTEGRATION_CONFLICT
    if FailureCategory.MISSING_CONTEXT in categories:
        return OutcomeStatus.BLOCKED, FailureAttribution.MISSING_CONTEXT
    if FailureCategory.CONTRACT in categories:
        return OutcomeStatus.BLOCKED, FailureAttribution.DECOMPOSITION_ERROR
    if FailureCategory.SEMANTIC_INCONCLUSIVE in categories:
        return OutcomeStatus.INCONCLUSIVE, FailureAttribution.VERIFICATION_INCONCLUSIVE
    return OutcomeStatus.FAILED, FailureAttribution.WORKER_ERROR
...
def attribute_outcome(
    *,
    ...
    if verification is not None:
        ...
        if verification.accepted:
        ...
        elif (structured := _classify_structured_failures(verification)) is not None:
            status, attribution = structured
            ...
            if structured_checks & {"file_exists", "evidence_requirements"}:
                attribution = FailureAttribution.MISSING_CONTEXT
            elif "integration_conflict" in structured_checks:
                attribution = FailureAttribution.INTEGRATION_CONFLICT
            else:
                attribution = FailureAttribution.WORKER_ERROR
    else:
