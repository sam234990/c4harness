...
const state = {
  lang: localStorage.getItem("c4harness-lang") || (navigator.language.startsWith("zh") ? "zh" : "en"),
...
async function init() {
  ...
  $("#lang-button").addEventListener("click", async () => { state.lang = state.lang === "zh" ? "en" : "zh"; localStorage.setItem("c4harness-lang", state.lang); applyLanguage(); await Promise.all([loadOverview(), loadChart(), loadRecent(), loadAsyncTasks()]); if ($("#page-calls").classList.contains("active")) loadCalls(); });
