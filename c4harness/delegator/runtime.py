        ...
        *,
        ...
        if execute:
            ...
            try:
                ...
                # A verifier or hook failure is an orchestration/runtime
                # failure, not evidence that the selected worker lacks the
                # requested capability.  Convert it into an auditable result
                # so the ledger and graph report do not silently lose the
                # attempt.
                verification = VerificationResult(
                    accepted=False,
                    confidence="blocked",
                    issues=[
                        "[blocked] orchestration_lifecycle: "
                        f"{type(lifecycle_error).__name__}: {lifecycle_error}"
                    ],
                    failures=[FailureRecord(
                        category=FailureCategory.ENVIRONMENT,
                        code="orchestration_lifecycle_error",
                        message=f"{type(lifecycle_error).__name__}: {lifecycle_error}",
                        phase_or_check="verifier_or_hook",
                        retryable=False,
                        blame="environment",
                    )],
                )
