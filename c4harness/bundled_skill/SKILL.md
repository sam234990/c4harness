...
6. Invoke C4Harness from the repository being examined. Prefer Claude CLI when the user requests it, when a patch proposal is needed, or when no Codex provider is configured. For analysis:
...
`.c4harness/proposals/<task>.json` using this shape:
...
After the proposal is accepted, preview its execution without invoking any
Worker:
...
c4harness graph-run \
  --plan-file .c4harness/proposals/<task>.json \
  --repo "$PWD" \
  --max-parallel <1-or-small-bounded-value> \
  --parent-task-label "<same concise parent task>" \
  --json
```

For real execution, first confirm every external destination and path against
the proposal's risk manifests and the consent scope. Then add `--execute` and,
for approved private Claude work, `--external-policy allow
--data-classification private`. Keep `--max-parallel 1` unless the graph has
independent ready nodes. C4Harness may run independent Worker subprocesses in
parallel, but nodes with equal or ancestor/descendant write paths are
serialized. Patch integration and post-integration verification use short
serialized transactions in the shared graph workspace. Inspect the final root
verification; a successful worker process alone is not graph acceptance.
...
5. Terminal/significant events are always written to the durable local Inbox. Do not claim that this wakes the visible Codex UI. Inspect with `c4harness async-task inbox --unread-only` and acknowledge handled items with `c4harness async-task ack <inbox-id>`.
...
7. When the host or user surfaces an Inbox event, inspect `c4harness async-task status <task-id>` and `events <task-id>`, then decide whether to report completion, diagnose, patch, or explicitly start a replacement workload.
...
- `c4harness decompose` previews and records a contract graph; this command itself does not execute workers.
- `c4harness graph-run` compiles the same version-1 proposal and provides a dry-run preview by default. `--execute` runs assigned nodes and returns nonzero unless Root Verification accepts the graph.
- Graph execution supports dependency-aware bounded parallel batches. Independent ready nodes may run concurrently; overlapping declared write sets serialize deterministically.
- The integration module provides a Git-independent graph-scoped workspace where patches are applied in transactional copies with `apply → post-integration verify → commit/rollback`. A single task graph shares one canonical integration workspace; predecessor nodes are verified and committed before successors read the updated version.
- Structured verifier failures include stable category/code/message/check/retryability fields while retaining legacy issue text. Each node receives at most two total attempts by default; retryable worker or deterministic failures may use the same Worker or the next eligible fallback. Environment, permission, policy, consent, and integration-conflict failures do not retry and are excluded from negative Worker capability history.
...
- Graph execution recompiles the Codex proposal against the current Worker manifest and History before running; it does not execute an old assignment snapshot blindly.
- Graph backend selection follows compiled Worker assignments and eligible fallback candidates. The single-task `run` command remains explicit.
